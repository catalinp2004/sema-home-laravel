<template>
    <div class="hero-home position-relative" v-if="building">
        ...
        <ApartmentFilter v-if="building && building.floors && building.floors.length" v-model="roomsFilter"
            class="position-absolute bottom-0 start-0 mb-4 ms-4" />
    </div>
</template>
...
function setTitle(floor) {
    ...
    const filt = roomsFilter.value && roomsFilter.value !== 'toate' ? (roomsFilter.value === '1' ? 'O cameră:' : String(roomsFilter.value) + ' camere:') : '';
